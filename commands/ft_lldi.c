...
void ft_lldi_reg(t_pc *pc, t_union *un)
{
	int one;
	int two;
	int three;
	...
	if (un->arg[1] == REG_CODE)
	{
		...
		if (one <= 16 && one > 0 && two <= 16 && two > 0 && three <= 16 && three > 0)
			pc->reg[three - 1] = (unsigned int)ft_get_int(un, ft_check_position((int)(pc->reg[one - 1] + pc->reg[two - 1]) + pc->curr_position), 4);
	...
	if (un->arg[1] == DIR_CODE)
	{
		...
		if (one <= 16 && one > 0 && three <= 16 && three > 0)
			pc->reg[three - 1] = (unsigned int)ft_get_int(un, ft_check_position(((int)pc->reg[one - 1] + two) + pc->curr_position), 4);
		pc->curr_position = pc->curr_position + 6;
	}
}

void ft_lldi_dir(t_pc *pc, t_union *un)
{
	int one;
	int two;
	int three;
	...
	if (un->arg[1] == REG_CODE)
	{
		...
		if (two <= 16 && two > 0 && three <= 16 && three > 0)
			pc->reg[three - 1] = (unsigned int)ft_get_int(un, ft_check_position((one + (int)pc->reg[two - 1]) + pc->curr_position), 4);
		pc->curr_position = pc->curr_position + 6;
	}
	if (un->arg[1] == DIR_CODE)
	{
		...
		if (three <= 16 && three > 0)
			pc->reg[three - 1] = (unsigned int)ft_get_int(un, ft_check_position((one + two) + pc->curr_position), 4);
		pc->curr_position = pc->curr_position + 7;
	}
}

void ft_lldi_ind(t_pc *pc, t_union *un)
{
	int one;
	int two;
	int three;
	...
	if (un->arg[1] == REG_CODE)
	{
		...
		three = un->map[ft_check_position(pc->curr_position + 5)].value ;
		if (two <= 16 && two > 0 && three <= 16 && three > 0)
			pc->reg[three - 1] = (unsigned int)ft_get_int(un, ft_check_position((one + pc->reg[two - 1]) + pc->curr_position), 4);
		pc->curr_position = pc->curr_position + 6;
	}
	if (un->arg[1] == DIR_CODE)
	{
