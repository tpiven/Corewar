...
void ft_lldi_reg(t_pc *pc, t_union *un)
{
	...
	if (un->arg[1] == REG_CODE)
	{
		...
		if (one <= 16 && one > 0 && two <= 16 && two > 0 && three <= 16 && three > 0)
			pc->reg[three - 1] = (unsigned int)ft_get_int(un, ft_check_position((pc->reg[one - 1] + pc->reg[two - 1]) + pc->curr_position), 4);
	...
	if (un->arg[1] == DIR_CODE)
	{
		...
		if (one <= 16 && one > 0 && three <= 16 && three > 0)
			pc->reg[three - 1] = (unsigned int)ft_get_int(un, ft_check_position((pc->reg[one - 1] + two) + pc->curr_position), 4);
