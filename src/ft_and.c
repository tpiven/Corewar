...
void ft_and_dir(t_pc *pc, t_union *un)
{
	unsigned int one;
	unsigned int two;
	int plus;
	...
	if (un->arg[1] == REG_CODE)
		two = pc->reg[un->map[ft_check_position(pc->curr_position + plus)].value - 1];
	if (un->arg[1] == DIR_CODE)
		two = (unsigned int)ft_get_int(un, ft_check_position(pc->curr_position + plus), 4);
	if (un->arg[1] == IND_CODE)
		two = (unsigned int)ft_get_int(un, ft_check_position(pc->curr_position + ((short)ft_get_int(un, ft_check_position(pc->curr_position + plus), 2))), 4);
	if (un->arg[1] == REG_CODE)
		plus += 1;
	if (un->arg[1] == DIR_CODE)
		plus += 4;
	...
	if ( pc->reg[un->map[ft_check_position(pc->curr_position + plus)].value - 1] == 0)
		pc->carry = 1;
	else
		pc->carry = 0;
...
void ft_and_ind(t_pc *pc, t_union *un)
{
	unsigned int one;
	unsigned int two;
	int plus;
...
void ft_and_reg(t_pc *pc, t_union *un)
{
	unsigned int one;
	unsigned int two;
	int plus;
